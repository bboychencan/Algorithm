...
```
2020/09/07
这一题，写完TLE，想了一会儿没想出最优解。 答案很巧妙，留做备份，看看过段时间再看这道题是否可以
想出来
```
...
```
2020/09/07
这一题很快用一个排序的方法解决了，但感觉肯定不是最优。 看了一下高票答案，是一种很巧妙的O(n)方法
希望下次再回顾的时候可以想出来.
```
...
2020/09/14
最近各种跪在medium题上。
这一题看完的思路就是暴力两个for循环，但一想肯定TLE。实现过果然TLE。
然后陷如沉思，最近不知道是脑力不足还是怎么了，想了一会儿就感觉犯困，撑不住了
我一直在想既然是求所有的两两组合，肯定这个信息可以帮助减少一些计算。

我一直把这个问题当作一个图来思考，需要计算两两点之间的距离。 然后我就想
每一个点和所有其他点的距离之和有什么特别的地方，是否可以减少一些计算。
然后发现无果，脑子开始陷入崩溃，想哭。躺床上几分钟，哭完又回来，再想还是无果
我陷入了这个计算每个单点到所有点的距离如何优化这个问题上，然后发现完全没有
思路，我这个的心理状态是，肯定是脑子不好使了，不敢想复杂的情形，再努力想想
这个思路如何缩减计算次数，然后发现没有想法，然后就硬憋，然后实在崩不住了，
崩溃，看答案。 

。。。
一看投票答案，又是情景重现，怎么这么简单！！！但是思路非常巧妙。
它利用了每一位bit是独立的这个特点，然后只需要记录每一位bit在所有数中有多少是
1,多少是0,然后由于hamming距离只计算不同的，所以对于该位bit，对总值的贡献就
是1的个数 * 0的个数。 然后对每一位进行同样的计算，加起来。。。

这个思路我是没想到，而且一旦想到确实很简单。。。 
那么现在的问题就是我为什么想不到这个思路，我为什么每次做不出来都是死死陷入一
个思路，死活拔不出来？ 我为什么在想问题的时候在一个方向行不通的时候及时跳出
圈子，跳到另一个思路呢？？？ 为什么？ 为什么？
```